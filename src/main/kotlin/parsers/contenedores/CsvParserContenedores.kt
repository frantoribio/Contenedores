...
class CsvParserContenedores : Parser<ContenedorDto> {
    fun mapTo(input: Sequence<String>): Sequence<ContenedorDto> = input.drop(1).map { line ->
    ...
    fun mapFrom(input: Sequence<ContenedorDto>): Sequence<String> = sequence {
        yield(
        ...
        yieldAll(input.map { contenedores ->
